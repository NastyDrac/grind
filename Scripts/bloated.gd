...
func apply_condition(who : Enemy, condition : Condition):
	...
	if existing_bloated == null:
		# Enemy doesn't have bloated yet, add new instance
		var new_bloated = self.duplicate(true)
		new_bloated.enemy = who
		new_bloated.stacks = condition.stacks
		who.condition.append(new_bloated)
		
		# Connect to enemy_dies signal for this new instance
		Global.enemy_dies.connect(new_bloated.on_enemy_dies)
	else:
		# Enemy already has bloated, add stacks
		existing_bloated.stacks += condition.stacks

func _get_existing_bloated(who: Enemy) -> Bloated:
